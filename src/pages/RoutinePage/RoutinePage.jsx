...
const toRoutine = (routine) => ({
  id: routine.id,
  name: routine.title,
  repeatType: routine.repeatType === 'DAILY' ? 'daily' : routine.repeatType === 'COUNT' ? 'count' : 'days',
  days: routine.repeatDays
    ? routine.repeatDays.split(',').map((day) => apiDayToDay[day]).filter(Boolean)
    : [],
...
const RoutinePage = () => {
  ...
  const fetchRoutines = useCallback(async (day, showLoading = true) => {
    ...
    try {
      ...
      const response = await axios.get(`${BASE_URL}/api/v1/routinefit/routines`, {
        headers: {
      ...
      setRoutines(Array.isArray(response.data.data) ? response.data.data.map(toRoutine) : []);
    } catch (error) {
      setRoutines([]);
      setFetchError(
  ...
  const sortedRoutines = useMemo(
    () => routines.toSorted((first, second) => first.time.localeCompare(second.time)),
    [routines],
  );
  ...
  const handleToggleRoutine = (routineId) => setRoutines((items) => items.map((routine) => (
