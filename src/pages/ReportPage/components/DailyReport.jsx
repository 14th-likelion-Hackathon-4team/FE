const RoutineStatus = ({ status }) => {
  ...
  return (
    <span
      aria-label="미완료"
      className="size-[22px] shrink-0 rounded-full border border-dashed border-text-muted"
    />
  );
};
...
const DailyReport = ({ report }) => {
  return (
    <section aria-labelledby="daily-report-title" className="mt-[22px] w-full">
      ...
      >
        <span
          className="h-full bg-disabled"
          style={{ width: `${report.progress.plan}%` }}
        />
        <span
          className="h-full bg-primary"
          style={{ width: `${report.progress.alternative}%` }}
        />
      </div>
